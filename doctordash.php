<?php
    ...
    //$_SESSION['email']='jairbohara999@gmail.com';
    session_start();
    $isDoc=$_SESSION['isdoctor'];
    if(!isset($_SESSION['email']) || $isDoc!=1)
    {
    ...
    {
        $sql = mysqli_query($con, "SELECT * FROM doctors where email='$email' and isVerified=1 and isEmailVerified=1");
    ...
    if(isset($_POST['id']))
    {
        ...
        if(empty($reason))
        {
            ...
            $uemail=$data['email'];
            $s = mysqli_query($con, "SELECT * FROM users where email='$uemail'");   
            ...
            $date=$data['date'];
            $d_id=$data['d_id'];
            $w = mysqli_query($con, "SELECT * FROM doctors where id='$d_id'");   
            $d = mysqli_fetch_array($w);
            $dname=$d['name'];
            $uname=$datad['name'];
            $rsn=$_POST['reason'];
            $mail->Body = "
                Hello $uname,<br>Your Appointment dated: $date With doctor : Dr. $dname, has been canceled for the following reason(s):<br><br>
...
<header style="height: 15%; margin-bottom:50px;">
	...
	<ul class="navigation">
		...
		<li><a style="display:flex; white-space:nowrap; color:#f44336;" href="doctordash.php"><i class="fa fa-id-card"></i>&nbsp;Appointments</a></li>
...
<div class="container" style="margin-top:-20px;">
  ...
  <ul class="responsive-table">
    <li class="table-header">
      <div class="col col-1" style="display: flex; align-items: center; justify-content: center; padding:5px;">Name</div>
    ...
    <div id="appointmentstabledoc">
        <?php
        $sql = mysqli_query($con, "SELECT * FROM appointments where d_id='$d_id' and isactive=1");
        ...
        if($("#reason"+id).val()===""){
            ...
            $.ajax({
                ...
                success : function(data) {
                    console.log(data);
                    $('#appointmentstabledoc').replaceWith($('#appointmentstabledoc',data));
                }
