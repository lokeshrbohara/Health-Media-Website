<?php
    ...
    {
        $id=$_POST['id'];
        $con->query("UPDATE doctors set isVerified=0 where id='$id'");
        $sql = mysqli_query($con, "SELECT * FROM doctors where isEmailVerified=1 and isVerified=0 and id='$id'");
        if($data = mysqli_fetch_array($sql))
        $con->query("DELETE FROM users WHERE email='".$data['email']."'");
...
<a href="#" class="navbar-brand"><img src="icon.png" style="max-width:10%;"alt="Brand Logo"> Remedium Admin</a>
    ...
    <div style="justify-content: flex-end;" class="collapse navbar-collapse" id="navbarCollapse">
        <div class="navbar-nav">
            <a href="http://localhost/ip/admin.php" class="nav-item nav-link">Unverified</a>
            <a href="http://localhost/ip/verifieddoc.php" class="nav-item nav-link active">Verified</a>
...
<div style="margin-top:50px;" class="container">
  ...
  <ul class="responsive-table">
    <li class="table-header">
      ...
      <div class="col col-9" style="display: flex; align-items: center; justify-content: center; padding:5px;">Unverify</div>
    </li>
    
    <?php

    $sql = mysqli_query($con, "SELECT * FROM doctors where isEmailVerified=1 and isVerified=1");
    ...
    <li class="table-row">
      ...
      <div class="col col-9" style="display: flex; align-items: center; justify-content: center;" data-label="Verify"><button id="btn<?php echo $data['id'] ?>" onclick="unverify(<?php echo $data['id']?>)">Unverify</button></div>
    </li>
...
<script>
    ...
    function unverify(id){
        ...
        if (confirm("You are Setting the Status of Doctor as Unverified.")) {
